...
└── .agent/
    ...
    ├── skills/                  # 36 Skills (including 29 modes)
...
As "Regras de Ouro" são diretrizes inalteráveis que definem a integridade do FusionONE.

1. **MFA Integrity (Lei 80):** O fluxo `Credentials -> MFA -> Dashboard` é atômico e monitorado em background. O bypassing de segurança ou a degradação da experiência de login (spinner infinito) são proibidos.
2. **29 Precedence:** Toda alteração estrutural exige auditoria e aprovação sequencial do Administrador Master.
